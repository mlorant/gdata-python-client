...
class XmlElement(object):
  ...
  def get_elements(self, tag=None, namespace=None, version=1):
    ...
    if elements:
      for qname, element_def in list(elements.items()):
  ...
  def get_attributes(self, tag=None, namespace=None, version=1):
    ...
    if attributes:
      for qname, attribute_def in list(attributes.items()):
    ...
    for qname, value in list(self._other_attributes.items()):
      if _qname_matches(tag, namespace, qname):
    ...
    for element in tree:
      if elements and element.tag in elements:
        ...
        self._other_elements.append(_xml_element_from_tree(element, XmlElement,
                                                           version))
    for attrib, value in list(tree.attrib.items()):
  ...
  def _attach_members(self, tree, version=1, encoding=None):
    ...
    if elements:
      for tag, element_def in list(elements.items()):
        member = getattr(self, element_def[0])
    ...
    if attributes:
      for attribute_tag, member_name in list(attributes.items()):
    ...
    for key, value in list(self._other_attributes.items()):
