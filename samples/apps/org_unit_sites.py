...
def main():
  ...
  while not email:
    email = eval(input('Please enter admin email address (admin@example.com): '))
  while not password:
  ...
  while not domain:
    ...
    choice = eval(input('You have not given us the domain name. ' +
                        'Is it %s? (y/n)' % (domain)))
    if choice == 'n':
      domain = eval(input('Please enter domain name (domain.com): '))
